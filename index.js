...
if (conf.AUTOBIO === 'yes') {
    setInterval(() => {
      ...
      zk.updateProfileStatus(
        `${conf.OWNER_NAME} is active 24/7 ${date.toLocaleString('en-US', { timeZone: 'Africa/Tanzania' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Tanzania' })}.`
      );
    }, 10 * 1000);
  }
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  let auto_reply_message = `Hello @${senderNumber}, am hans-md chat bot how can help my friend cause my owner is unavailable right now🤦‍♂️. \n\n> How can help today\n`🤔`?`;
...
function createNotification(deletedMessage) {
  ...
  let notification = `*⚠️HANS-MD ANTIDELETE⚠️*\n\n`;
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "255760774888", '255756530143', '255760774888', "255760774888", '255692540143', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['255756530143', '255760774888', "255760774888", '255692540143'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
      ...
      if (! superUser && origineMessage === auteurMessage && conf.AUTO_BLOCK === 'yes') {
        zk.sendMessage(auteurMessage, {
          'text': "⚠️am blocking you because you have violated hans policies⚠️!"
        });
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (!superUser) {
    return repondre("Only for my owner or HansTz to execute this command ⚠️");
  }
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or Sir HansTz to execute ⚠️`;
    ...
    await zk.sendMessage(origineMessage, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          body: "Regards HansTz",
          sourceUrl: conf.GURL,
          thumbnailUrl: "https://files.catbox.moe/4bvh1g.jpg" || conf.BOT_MENU_LINK,
          mediaType: 1,
  ...
  var kikmsg = `bot detected ;you will be remove because of reaching warn-limit`;
    ...
    zk.ev.on('group-participants.update', async group => {
      ...
      try {
        ...
        ppgroup = 'https://files.catbox.moe/4bvh1g.jpg';
      }
      try {
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        ...
        fs.readdirSync(__dirname + "/Hans_Cmds").forEach(fichier => {
          if (path.extname(fichier).toLowerCase() == ".js") {
            try {
              require(__dirname + "/Hans_Cmds/" + fichier);
